...
def walk_forward_forecasting(series, periods_df):
    ...
    with warnings.catch_warnings():
        ...
        for _, row in tqdm(periods_df.iterrows(),
            ...
            # window_end_period = row["window_end"]
...
def process_forecast_accuracy(name, plot_show=True, col_name=""):
    ...
    df_accuracy = pd.read_pickle(f"data/clean/SARIMA_RollWalkForward/updated/accuracy/df_accuracy_{name}.pickle")[["actual", "forecast"]]

    # Load the deseasonalized data
    df_deseasonalized = pd.read_pickle(f"data/clean/deseasonalised/{name}.pickle")
    ...
    last_seasonal_factor = df_seasonal_factor["seasonal_factor"]
    last_seasonal_factor = last_seasonal_factor[last_seasonal_factor.index.month == forecast_date.month].rolling(5).mean().shift(1).iloc[-1]
    df_seasonal_factor.loc[forecast_date] = last_seasonal_factor

    # Load the actual values
    df_actual = pd.read_pickle("data/clean/total_export.pkl")[[name]]
